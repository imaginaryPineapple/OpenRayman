...
namespace openrayman
{
        ...
        m_valid(false)
    {
        ...
        if(!file::exists(m_location))
        {
            ...
            if(name == "rayman2")
                message_box::display("[openrayman::game::game (constructor)] Error!", "Rayman 2: The Great Escape could not be found in the user or data directory."
                    "\nRefer to the the file \"README.md\" or use the command line option \"--help\" for further instructions.", true);
            else
                message_box::display("[openrayman::game::game (constructor)] Error!", "The game \"" + name + "\" could not be found in the user or data directory."
        ...
        if(file::exists(m_location + "/manifest.json"))
        {
            ...
            if(manifest_stream.is_open())
            {
                ...
                {
                    message_box::display("[openrayman::game::game (constructor)] Error!", "The manifest of the game \"" + name + "\" lacks one or more required properties.", true);
                    return;
                }
        ...
        else
        {
            // Show a special message if we can't find "rayman2".
            if(name == "rayman2")
                message_box::display("[openrayman::game::game (constructor)] Error!", "Rayman 2: The Great Escape has no valid manifest."
                ...
                message_box::display("[openrayman::game::game (constructor)] Error!", "The game \"" + name + "\" has no valid manifest.", true);
        }
    }
    ...
    const std::string game::find_file(const std::string& file, bool show_error_msg) const
    {
        ...
        if(show_error_msg)
            message_box::display("[openrayman::game::find_file] Error!", "The file \"" + file + "\" could not be found in the game \"" + m_info.name + "\"."
